...
Membership is **not** refreshed per request, because Airflow 3.0.x does not expose a synchronous refresh hook on `BaseAuthManager`, so a user's authorization is stable for the cookie's TTL (1 hour by default).
