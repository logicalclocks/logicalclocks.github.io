...
!!! notice "Recommended approach if `/hopsfs` is mounted"
    ...
    If the file system is not mounted, then project files can be localized using the [download api][hopsworks_common.core.dataset_api.DatasetApi.download] to localize files in the current working directory.
