...
Hopsworks support scheduling jobs to run on a regular basis,
...
Next step is to select the Jupyter Notebook to run.
...
Arguments must be in the format of `-p arg1 value1 -p arg2 value2`.
For each argument, you must first provide `-p`, followed by the parameter name (e.g. `arg1`), followed by its value (e.g. `value1`).
