...
This applies when you run `hops session push` from the root directory of the repository.
From anywhere else the command says it is not a git repository, skips the sync and pushes the session without a checkout.
