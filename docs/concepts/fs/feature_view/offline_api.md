...
The materialized data is partitioned by each row's UTC event date — at day, month or year precision — making the dataset time-addressable: reads can select a time range instead of the whole dataset, and batches may arrive in any order, so backfills and late-arriving events land in the partitions their rows belong to.
For time-series models this replaces materialized time-series splits — the train/test boundary is chosen at read time, so both windows grow or shift automatically as new batches arrive (for example, a test set that is always the most recent 30 days, or a training window shifted after detecting model drift).
