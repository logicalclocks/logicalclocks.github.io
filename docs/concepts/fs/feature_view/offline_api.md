...
The materialized data is partitioned by each row's event time at day granularity, making the dataset time-addressable: reads can select a time range instead of the whole dataset, and batches may arrive in any order — backfills and late-arriving events land in the day partitions their rows belong to.
